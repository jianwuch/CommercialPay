...
public abstract class Y_DividerItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public void onDraw(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            Y_Divider divider = getDivider(itemPosition, isLast);
            if (null != divider) {
                if (divider.getLeftSideLine().isHave()) {
                    int lineWidthPx =
                            Dp2Px.convert(context, divider.getLeftSideLine().getWidthDp());
                    int startPaddingPx =
                    ...
                    drawChildLeftVertical(child, c, parent, divider.getLeftSideLine().getColor(),
                            lineWidthPx, startPaddingPx, endPaddingPx);
                }
                if (divider.getTopSideLine().isHave()) {
                    ...
                    drawChildTopHorizontal(child, c, parent, divider.topSideLine.getColor(),
                            lineWidthPx, startPaddingPx, endPaddingPx);
                }
                if (divider.getRightSideLine().isHave()) {
                    int lineWidthPx =
                            Dp2Px.convert(context, divider.getRightSideLine().getWidthDp());
                    int startPaddingPx =
                    ...
                    drawChildRightVertical(child, c, parent, divider.getRightSideLine().getColor(),
                            lineWidthPx, startPaddingPx, endPaddingPx);
                }
                if (divider.getBottomSideLine().isHave()) {
                    int lineWidthPx =
                            Dp2Px.convert(context, divider.getBottomSideLine().getWidthDp());
                    int startPaddingPx =
                    ...
                    drawChildBottomHorizontal(child, c, parent,
                            divider.getBottomSideLine().getColor(), lineWidthPx, startPaddingPx,
                            endPaddingPx);
                }
            }
        }
    ...
    private void drawChildBottomHorizontal(View child, Canvas c, RecyclerView parent,
            @ColorInt int color, int lineWidthPx, int startPaddingPx, int endPaddingPx) {
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent,
        ...
        Y_Divider divider = getDivider(itemPosition, false);

        if (divider == null) {
            divider = new Y_DividerBuilder().create();
        }

        int left = divider.getLeftSideLine().isHave() ? Dp2Px.convert(context,
                divider.getLeftSideLine().getWidthDp()) : 0;
        int top = divider.getTopSideLine().isHave() ? Dp2Px.convert(context,
                divider.getTopSideLine().getWidthDp()) : 0;
        int right = divider.getRightSideLine().isHave() ? Dp2Px.convert(context,
                divider.getRightSideLine().getWidthDp()) : 0;
        int bottom = divider.getBottomSideLine().isHave() ? Dp2Px.convert(context,
                divider.getBottomSideLine().getWidthDp()) : 0;

        outRect.set(left, top, right, bottom);
    }

    public abstract @Nullable
    Y_Divider getDivider(int itemPosition, boolean isLast);
}
